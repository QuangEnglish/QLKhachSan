...
public class QuanLyPhong extends JFrame {
	...
	public static QuanLyPhong getInstance() throws SQLException {
	...
	private void layRaPhongHienTai() throws SQLException {
		conn = cn.getConnection();
		try {
	...
	// sửa
	private void xuLySưa() throws SQLException {
		conn = cn.getConnection();
		try {
	...
	// thêm
	private void xuLyThem() throws SQLException {
		int kq = 0;
		conn = cn.getConnection();
		try {
			if (textField.getText().equals("") || textField_1.getText().equals("")) {
